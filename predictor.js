...
const fileContent = fs.readFileSync('cloud.json', 'utf-8');
...
const loadModel = async () => {
  ...
  await storage
    .bucket(BUCKET_NAME)
    .file(`${MODEL_PATH}/model.json`)
    .download({ destination: modelJsonPath });

  // Download all the shard files (e.g., group1-shardXofY.bin)
  const shardFiles = await storage
    .bucket(BUCKET_NAME)
    .getFiles({ prefix: `${MODEL_PATH}/group1-shard` });

  // Ensure shard files are downloaded correctly
  await Promise.all(
    shardFiles[0].map((file) =>
      file.download({
        destination: path.join(tempDir, path.basename(file.name)),
      })
    )
  );
...
const preprocessImage = (imageBuffer) => {
  return tf.node
    .decodeImage(imageBuffer) // Decode image buffer menjadi tensor
